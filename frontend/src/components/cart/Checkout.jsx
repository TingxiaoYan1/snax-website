...
export default function Checkout() {
  ...
  const selectedCoupon =
    useSelector((s) => s.couponUse?.selectedCoupon) || null;
  ...
  const {
    ...
    const itemsPriceRaw = cartItems.reduce(
    ...
    const isPct =
      ...
      (selectedCoupon && selectedCoupon.percentage != null);
    ...
    const discountAmount = Number(
      Math.min(rawDiscount, isFinite(maxDed) ? maxDed : rawDiscount).toFixed(2)
    );
    ...
    const totalPrice = Number(
      (taxableBase + shippingPrice + taxPrice).toFixed(2)
    );
  ...
  // ----- BACKENDS -----
  const [createNewOrder, { isLoading: placingCOD }] =
    useCreateNewOrderMutation();
  const [createSqCheckout, { isLoading: startingCard }] =
  ...
  const couponFields = useMemo(() => {
    ...
    if (selectedCoupon.code)
      return { couponCode: String(selectedCoupon.code).toUpperCase() };
    return {};
  }, [selectedCoupon]);
  ...
  const submitCheckout = async (e) => {
    ...
    try {
      ...
      const msg =
        err?.data?.message || err?.error || err?.message || "Checkout failed";
      toast.error(msg);
    }
  };
          ...
          >
            ...
            <div className="mb-3">
              ...
              <select
                ...
                value={country} // will always be "United States"
                onChange={() => {}} // no-op (keeps it effectively read-only)
                required
              >
            ...
            <div className="row">
              <div className="mb-3 col-12 col-md-6">
                <label className="form-label" htmlFor="first_field">
                  First Name
                </label>
                <input
                  id="first_field"
              ...
              <div className="mb-3 col-12 col-md-6">
                <label className="form-label" htmlFor="last_field">
                  Last Name
                </label>
                <input
                  id="last_field"
            ...
            <div className="mb-3">
              <label className="form-label" htmlFor="apartment_field">
                Apartment (optional)
              </label>
              <input
                id="apartment_field"
            ...
            <div className="row">
              <div className="mb-3 col-12 col-md-6">
                <label className="form-label" htmlFor="zip_field">
                  ZIP / Postal Code
                </label>
                <input
                  id="zip_field"
              ...
              <div className="alert alert-success">
                ...
                selectedCoupon.percentage != null ? (
                  <>
                    ...
                    {selectedCoupon.maxDeduction != null
                      ? `, max $${money(selectedCoupon.maxDeduction)}`
                      : ""}
                    )
                  </>
                ) : (
                  <>
                    — Free Gift {giftQty} × <code>{giftProductId}</code>{" "}
                    (threshold ${money(giftThreshold)})
                  </>
                )}
              </div>
            ) : (
              <div className="alert alert-info small">
                No coupon selected yet. Pick one on the right.
              </div>
            )}
            ...
            <button
              ...
              disabled={!cartItems.length || placingCOD || startingCard}
            >
              {placingCOD || startingCard
                ? "Starting checkout…"
                : "PLACE ORDER"}
            </button>
          </form>
          ...
          <div className="shadow rounded bg-body p-4">
            ...
            {isFreeGift && giftProductId && (
              ...
              >
                <div className="d-flex justify-content-between align-items-center mb-2">
                  ...
                  <span
                    className={`badge ${
                      meetsGift ? "text-bg-success" : "text-bg-warning"
                    }`}
                  >
                    {meetsGift
                      ? "Qualified"
                      : `Spend $${money(
                          Math.max(0, giftThreshold - itemsPrice)
                        )} more`}
                  </span>
                </div>
                ...
                {giftLoading ? (
                  ...
                  <div className="row align-items-center">
                    ...
                    <div className="col-9 col-lg-7">
                      <Link to={`/product/${giftProduct._id}`}>
                        {giftProduct?.name}
                      </Link>
                ...
                <div className="small text-muted mt-2">
                  The gift will be added to your order after payment if you meet
                  the threshold.
                </div>
              </div>
            )}
            ...
            <div className="mb-3">
              ...
              >
                ...
                {myCoupons.map((c) => (
                  <option value={c._id} key={c._id}>
                    {c.code} ·{" "}
                    {c.type === "free_gift"
                      ? `FREE-GIFT (≥ $${money(c.threshold)} → ${c.giftQty} × ${
                          c.giftProduct
                        })`
                      : `${c.percentage}% off${
                          c.maxDeduction != null
                            ? ` (cap $${money(c.maxDeduction)})`
                            : ""
                        }`}{" "}
              ...
              <small className="text-muted">
                Don’t see your code?{" "}
                <Link to="/me/fetch_coupon">Claim it here</Link>.
              </small>
            </div>
            ...
            {isFreeGift && (
              <div
                className={
                  "alert " + (meetsGift ? "alert-success" : "alert-warning")
                }
              >
                {meetsGift ? (
                  <>
                    You qualify for the free gift (threshold $
                    {money(giftThreshold)} met).
                  </>
                ) : (
                  <>
                    Spend ${money(Math.max(0, giftThreshold - itemsPrice))} more
                    to qualify for the free gift.
                  </>
                )}
                <div className="small text-muted">
                  Gift is added after payment (in our system, not on Square).
                </div>
              </div>
            )}
            ...
            <div className="pt-2">
              ...
              <p>
                Subtotal:{" "}
                <span className="float-end">${money(itemsPrice)}</span>
              </p>
              {discountAmount > 0 && (
                <p>
                  Discount:{" "}
                  <span className="float-end">- ${money(discountAmount)}</span>
                </p>
              )}
              <p>
                Shipping (est.):{" "}
                <span className="float-end">${money(shippingPrice)}</span>
              </p>
              <p>
                Tax (est.):{" "}
                <span className="float-end">${money(taxPrice)}</span>
              </p>
              <hr />
              <p className="fw-bold">
                Total (est.):{" "}
                <span className="float-end">${money(totalPrice)}</span>
              </p>
              <div className="text-muted small">
                Final shipping & tax are calculated on the server. Free-gift
                coupons do not change the price — we add the gift item to your
                order after payment if you meet the threshold.
              </div>
            </div>
          </div>
        </div>
