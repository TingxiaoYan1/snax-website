...
/**
 * Admin "Update Product" with full Product model fields (except photos).
...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      stock: p.stock ?? "",
      seller: p.seller || "",
      expiredate: p.expiredate || "1 Month",
      ratings: p.ratings ?? 0,
    ...
    // Categories
    if (p.categories?.l1) {
      setL1(p.categories.l1);
      setL2(p.categories.l2 || "");
    } else if (catData?.l1?.length) {
    ...
    // Tags – ensure all schema keys exist
    const nums = {};
    (tagSchema?.numericKeys || []).forEach((k) => {
      nums[k] = p?.tags?.numbers?.[k] ?? 0;
    });
    const bools = {};
    (tagSchema?.booleanKeys || []).forEach((k) => {
      bools[k] = !!p?.tags?.booleans?.[k];
    });
    setTagNumbers(nums);
    setTagBooleans(bools);
  }, [data, catData, tagSchema]);
            ...
            <div className="row">
              <div className="mb-3 col">
                ...
                <select className="form-select" id="l1_field" name="l1" value={l1}
                        onChange={onChangeL1} disabled={!catData?.l1?.length}>
              ...
              <div className="mb-3 col">
                ...
                <select className="form-select" id="l2_field" name="l2" value={l2}
                        onChange={onChangeL2} disabled={!catData?.l1?.length || !l1}>
                  {l2Options.length === 0
